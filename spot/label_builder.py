#!/usr/bin/env python3
"""
Label builder for binary classification (0/1) for spot trend prediction.
Creates labels for predicting whether the next price movement will be up (1) or down (0).
Uses closing prices at time t and t+horizon for prediction.
"""
...
class LabelBuilder:
    ...
    def apply_regime_filter(self, df: pd.DataFrame) -> pd.DataFrame:
        """Filter data to higher-volatility regime using ATR quantile."""
        if 'price_true_range' not in df.columns:
            logger.warning("price_true_range not available; skipping regime filter")
            return df

        logger.info("Applying regime filter (ATR quantile)...")
        df = df.sort_values(['exchange', 'symbol', 'interval', 'time']).copy()

        atr = df.groupby(['exchange', 'symbol', 'interval'])['price_true_range'].transform(
            lambda x: x.rolling(self.regime_atr_window).mean()
        )
        df['atr_regime'] = atr
        atr_q = df.groupby(['exchange', 'symbol', 'interval'])['atr_regime'].transform(
            lambda x: x.quantile(self.regime_atr_quantile)
        )
        before = len(df)
        df = df[df['atr_regime'] >= atr_q]
        after = len(df)
        logger.info(f"Regime filter kept {after:,}/{before:,} rows (q={self.regime_atr_quantile})")
        df = df.drop(columns=['atr_regime'])
        return df

    def create_binary_labels(self, df: pd.DataFrame, threshold: float = None, horizon: int = None) -> pd.DataFrame:
        """
        ...
        Args:
            ...
            horizon: Bars ahead for label horizon
        ...
        # Calculate horizon close price for each group
        df['next_close'] = df.groupby(['exchange', 'symbol', 'interval'])['price_close'].shift(-horizon)

        # Calculate price change
        df['price_change'] = (df['next_close'] - df['price_close']) / df['price_close']

        # Create binary labels with threshold band
        df[self.label_col] = np.where(
            df['price_change'] > threshold,
            1,
            np.where(df['price_change'] < -threshold, 0, np.nan)
        )
    ...
    def prepare_training_data(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        # Drop consistently low-value features (based on prior importance=0)
        low_value_features = {
            'oi_mean_24',
            'ls_top_ratio',
        ...
        feature_cols = [col for col in feature_cols if col not in low_value_features]

        # Keep numeric features only (drop non-numeric like unit columns)
        numeric_cols = df[feature_cols].select_dtypes(include=['number']).columns.tolist()
        dropped_cols = sorted(set(feature_cols) - set(numeric_cols))
        if dropped_cols:
            logger.info(f"Dropping non-numeric feature columns: {dropped_cols}")
...
- Label Horizon: {self.label_horizon} bars
- Label Threshold: {self.label_threshold:.4f}
- Regime Filter: ATR q={self.regime_atr_quantile} (window={self.regime_atr_window})
...
def main():
    ...
    try:
        ...
        # Regime filter and create binary labels
        df = builder.apply_regime_filter(df)
