...
from DFLogerLib import DFLogger
import numpy as np
import pandas as pd
...
class XGBoostTradingAlgorithm(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.objectstore_prefix = "spot_btc_binance"
        ...
        self.model_api_url = f"{self.domain}/api/{self.model_version}/spot/latest/model"
        self.dataset_summary_api_url = f"{self.domain}/api/{self.model_version}/spot/latest/dataset-summary"
        ...
        try:
            ...
            self.min_order_quantity = float(getattr(props, "MinimumOrderSize", 0.0) or 0.0)
            if self.min_order_quantity <= 0:
                self.min_order_quantity = float(getattr(props, "LotSize", 0.0) or 0.0)
        except Exception:
        ...
        # Fallback ordering for spot model (price + taker + agg_taker + cross)
        self.model_features = [
            "price_open","price_high","price_low","price_close","price_volume_usd",
            "taker_aggregated_buy_volume_usd","taker_aggregated_sell_volume_usd",
            "agg_taker_aggregated_buy_volume_usd","agg_taker_aggregated_sell_volume_usd",
            "price_close_return_1","price_close_return_5","price_log_return",
            ...
            "agg_taker_buy_ratio","agg_taker_imbalance","agg_taker_buy_mean_12","agg_taker_sell_mean_12",
            "agg_taker_buy_std_12","agg_taker_sell_std_12","agg_taker_buy_zscore","agg_taker_sell_zscore",
            "cross_taker_price","cross_agg_taker_price",
            "cross_taker_imbalance_price","cross_agg_taker_imbalance_price"
        ]

        # ===== Strategy meta =====
        self.strategy_name = "Metode: Spot New Gen - DragonFortune"
        self.strategy_id = 40
        ...
        # ===== DFLogger init (standardized payloads) =====
        self.logger_base_url = str(getattr(self, "apidomain", "") or "").strip() or self.domain
        self.logger = DFLogger.init(
            algo=self,
            strategy_id=self.strategy_id,
            base_url=self.logger_base_url,
            is_backtest=not self.LiveMode
        )
        ...
        # ===== Trading parameters =====
        self.prediction_buy_threshold = 0.52
        self.prediction_sell_threshold = 0.48
        ...
        self.stop_loss_pct = 0.045
        self.take_profit_pct = 0.040
        ...
        if getattr(self, "logger", None) is not None:
            # Send a heartbeat once per day to indicate the algo is alive
            self.Schedule.On(
    ...
    def OnOrderEvent(self, orderEvent: OrderEvent):
        try:
            ...
            # Exit filled
                ...
                is_long = (self.entry_direction == 1)
                if self.entry_price is not None:
                    msg, real_tp, real_sl = self.FormatExitSignal(self.entry_price, fill_price, reason, is_long=is_long)
                else:
                    msg, real_tp, real_sl = self.FormatExitSignal(0.0, fill_price, reason, is_long=is_long)

                self.SendSignal("Signal Exit (Filled)", msg)

                exit_jenis = "exit_long" if is_long else "exit_short"
                self.sendAPI("/orders", {
                    "id_method":self.strategy_id, "datetime": self.Time.strftime("%Y-%m-%d %H:%M:%S"), 
                    "type": "exit",
                    "jenis": exit_jenis,
                ...
                if self.entry_price is not None:
                    try:
                        t = self.CalcEntryTargets(self.entry_price, is_long=is_long)
                        target_tp = float(t.get("tp_price", 0.0))
                        target_sl = float(t.get("sl_price", 0.0))
                        ratio = float(t.get("rr", 0.0))
                    except Exception:
                        target_tp = 0.0
                        target_sl = 0.0
                        ratio = 0.0
    ...
    def TradeLogic(self, pred: float):
        ...
        # Trend/volatility filters (spot)
        long_ok = True
        short_ok = self.enable_short
        ...
        if pred > self.current_buy_threshold and qty <= 0 and not self.pending_entry and not self.pending_exit and long_ok:
            ...
            if self._last_submit_signal_bar != self.entry_time or self._last_submit_signal_kind != "ENTRY":
                ...
                # Signal payload will be sent on filled order (OnOrderEvent) to ensure qty is accurate.
        ...
        # ENTRY SHORT
        elif pred < self.current_sell_threshold and qty >= 0 and not self.pending_entry and not self.pending_exit and short_ok:
            self.pending_entry = True
            self.entry_price = float(self.Securities[self.symbol].Price)  # provisional
            self.entry_time = self.Time
            self.entry_direction = -1
    ...
    def sendAPI(self, path: str, params: dict):
        """
        Standardized payload hook via DFLoggerLib.
        """
        try:
            if not getattr(self, "logger", None):
                self.Debug("[sendAPI] DFLogger not initialized")
                return False

            payload = params if params is not None else {}
            endpoint = (path or "").strip().lower()

            if endpoint.endswith("/logs"):
                return self.logger.log(message=str(payload.get("message", "")))

            if endpoint.endswith("/reminders"):
                return self.logger.send_reminder(message=str(payload.get("message", "")))

            if endpoint.endswith("/orders"):
                return self.logger.send_order(
                    type=payload.get("type", ""),
                    jenis=payload.get("jenis", ""),
                    price=float(payload.get("price", 0.0)),
                    quantity=float(payload.get("quantity", 0.0)),
                    balance=float(payload.get("balance", self.Portfolio.TotalPortfolioValue)),
                    message=str(payload.get("message", "")),
                )

            if endpoint.endswith("/signals"):
                try:
                    leverage = float(payload.get("leverage", self.Securities[self.symbol].Leverage))
                except Exception:
                    leverage = float(payload.get("leverage", 1.0) or 1.0)

                qty = payload.get("quantity", None)
                if qty is None:
                    try:
                        qty = float(self.Portfolio[self.symbol].Quantity)
                    except Exception:
                        qty = 0.0
                else:
                    qty = float(qty)
                ...
                if ratio is None:
                    try:
                        entry = float(payload.get("price_entry", 0.0) or 0.0)
                        tp = float(payload.get("target_tp", 0.0) or 0.0)
                        sl = float(payload.get("target_sl", 0.0) or 0.0)
                ...
                return self.logger.send_signal(
                    type=payload.get("type", ""),
                    jenis=payload.get("jenis", ""),
                    price_entry=float(payload.get("price_entry", 0.0)),
                    price_exit=float(payload.get("price_exit", 0.0)),
                    target_tp=float(payload.get("target_tp", 0.0)),
                    target_sl=float(payload.get("target_sl", 0.0)),
                    real_tp=float(payload.get("real_tp", 0.0)),
                    real_sl=float(payload.get("real_sl", 0.0)),
            ...
            self.Debug(f"[sendAPI] Unknown path: {path}")
            return False
