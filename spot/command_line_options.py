#!/usr/bin/env python3
"""
Command-line options handler for XGBoost spot pipeline.
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="XGBoost Spot Trading Pipeline with Database Filtering",
...
Examples:
  ...
  python load_database.py --time 1700000000000,1701000000000
    ...
    if not args.model_version:
        parser.error("--model-version is required (ex: spot_btc_binance or spot_eth_binance)")
...
class DataFilter:
    ...
    def get_pair_filter_sql(self, pair_col: str = 'pair', mode: str = 'pair') -> str:
        ...
        if self.pair_filter:
            if mode == 'base':
                base_pairs = [self._normalize_base_symbol(pair) for pair in self.pair_filter]
                return f"{quoted_col} IN ({','.join([repr(pair) for pair in base_pairs])})"
