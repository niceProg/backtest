#!/usr/bin/env python3
"""
Load data from futures trading tables with filtering capabilities.
Extracts data based on exchange, pair, interval, time, and days parameters.
Pure futures version (no spot tables).
"""
...
class DatabaseLoader:
    """Load data from futures tables with filtering capabilities."""
    ...
    def __init__(self, data_filter: DataFilter, output_dir: str = './output_train',
        ...
        self.tables = {
            'cg_futures_price_history': {
                ...
                'data_columns': ['open', 'high', 'low', 'close', 'volume_usd', 'base_asset'],
                'pair_mode': 'pair'
            },
            ...
            'cg_futures_aggregated_taker_buy_sell_volume_history': {
                ...
                'pair_col': 'symbol',
                'key_columns': ['time', 'exchange', 'symbol', 'interval'],
                'data_columns': ['aggregated_buy_volume', 'aggregated_sell_volume', 'unit', 'base_asset'],
                'pair_mode': 'base'
            },
            ...
            'cg_liquidation_aggregated_history': {
                ...
                'pair_col': 'symbol',
                'key_columns': ['time', 'symbol', 'interval'],
                'data_columns': ['aggregated_long_liquidation_usd',
        ...
        self._quote_assets = [
            "USDT", "USDC", "USD", "BUSD", "FDUSD", "TUSD", "USDP", "USDS", "DAI", "EUR"
        ]

    def _normalize_base_symbol(self, symbol: str) -> str:
        """Normalize pair/symbol to base asset for futures tables."""
        if not symbol:
            return symbol
    ...
    def load_all_tables(self) -> Dict[str, pd.DataFrame]:
        """Load data from all futures tables."""
        all_data = {}
    ...
    def load_data_from_csv_files(self, csv_dir: str) -> Dict[str, pd.DataFrame]:
        ...
        # Expected CSV filenames (futures)
        csv_files = {
            ...
            'cg_futures_aggregated_taker_buy_sell_volume_history': 'futures_taker_volume_history.csv',
            'cg_futures_aggregated_ask_bids_history': 'futures_ask_bids_history.csv',
            ...
            'cg_open_interest_aggregated_history': 'open_interest_aggregated_history.csv',
            'cg_liquidation_aggregated_history': 'liquidation_aggregated_history.csv'
        }
    ...
    def apply_filters_to_dataframe(self, df: pd.DataFrame, table_name: str) -> pd.DataFrame:
        ...
        # Exchange filter
        if self.data_filter.exchange_filter and table_info['exchange_col']:
        ...
        if self.data_filter.pair_filter:
            ...
            if table_info.get('pair_mode') == 'base':
                base_pairs = [self._normalize_base_symbol(pair) for pair in self.data_filter.pair_filter]
                filtered_df = filtered_df[filtered_df[pair_col].isin(base_pairs)]
            else:
...
def main():
    ...
    try:
        ...
        if all_data:
            ...
            logger.info("Ready for merge_9_tables.py")
        else:
