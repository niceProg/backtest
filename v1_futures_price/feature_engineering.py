#!/usr/bin/env python3
"""
Feature engineering for futures trading tables based on semantic nature of each dataset.
Implements futures-focused features for XGBoost training.
...
class FeatureEngineer:
    ...
    def load_merged_data(self) -> pd.DataFrame:
        ...
        merged_file = datasets_dir / 'merged_price_only.parquet'

        if not merged_file.exists():
            # Fallback to root directory for backward compatibility
            merged_file = self.output_dir / 'merged_price_only.parquet'

        if not merged_file.exists():
            logger.error(f"Merged data file not found: {merged_file}")
            logger.error(f"Checked: {datasets_dir / 'merged_price_only.parquet'} and {self.output_dir / 'merged_price_only.parquet'}")
            sys.exit(1)
    ...
    def add_orderbook_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        total_usd = df['ob_aggregated_bids_usd'] + df['ob_aggregated_asks_usd']
        total_qty = df['ob_aggregated_bids_quantity'] + df['ob_aggregated_asks_quantity']

        df['ob_imbalance_usd'] = (df['ob_aggregated_bids_usd'] - df['ob_aggregated_asks_usd']) / total_usd
        df['ob_imbalance_qty'] = (df['ob_aggregated_bids_quantity'] - df['ob_aggregated_asks_quantity']) / total_qty
        df['ob_total_usd'] = total_usd
        df['ob_total_qty'] = total_qty
        df['ob_range_percent'] = df.get('ob_range_percent', np.nan)

        self.feature_columns.extend([
            'ob_imbalance_usd', 'ob_imbalance_qty', 'ob_total_usd',
            'ob_total_qty', 'ob_range_percent'
        ])
    ...
    def add_liquidation_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['liq_imbalance'] = (df['liq_aggregated_long_liquidation_usd'] -
                               df['liq_aggregated_short_liquidation_usd']) / total_liq
