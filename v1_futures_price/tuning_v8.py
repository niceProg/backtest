...
class XGBoostTradingAlgorithm(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.volatility_min = 0.003
        self.range_min = 0.0015
        self.momentum_min = 0.0015
    ...
    def TradeLogic(self, pred: float):
        ...
        if pred > self.current_buy_threshold and qty <= 0 and not self.pending_entry and not self.pending_exit:
            try:
                ...
                true_range = float(self.last_feature_map.get("price_true_range", 0.0))
                close = float(self.last_feature_map.get("price_close", 0.0))
                ret_5 = float(self.last_feature_map.get("price_close_return_5", 0.0))
                range_ratio = (true_range / close) if close > 0 else 0.0
                if vol_5 < self.volatility_min or range_ratio < self.range_min or ret_5 <= self.momentum_min:
                    return
            except Exception:
                return
