...
class XGBoostTradingAlgorithm(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.min_hold_bars = 10  # minimum holding bars before prediction-based exit
        ...
        self.fee_rate_per_side = 0.0004
        self.fee_buffer_mult = 2.0
        self.min_pred_exit_pnl_pct = self.fee_rate_per_side * 2.0 * self.fee_buffer_mult
        self.max_hold_bars = 48
        self.time_stop_loss_pct = -0.01
        self.trailing_start_pct = 0.02
        self.trailing_offset_pct = 0.01
        self.volatility_min = 0.002
    ...
    def CheckStopLossTakeProfit(self, price: float) -> bool:
        try:
            ...
            try:
                if self.entry_time is not None and self.max_hold_bars:
                    if self.Time >= (self.entry_time + timedelta(hours=int(self.max_hold_bars))):
                        if pnl_pct <= self.time_stop_loss_pct:
                            self.pending_exit = True
            ...
            try:
                if pnl_pct >= self.trailing_start_pct and self.highest_price_since_entry:
                    trail_price = self.highest_price_since_entry * (1.0 - self.trailing_offset_pct)
                    if price <= trail_price:
                        self.pending_exit = True
                        self.last_exit_reason = "TRAIL"
                        self.Liquidate(self.symbol)
                        return True
            except Exception:
                pass
