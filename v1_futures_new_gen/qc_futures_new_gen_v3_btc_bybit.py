...
class XGBoostTradingAlgorithm(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        # ===== Strategy meta =====
        self.strategy_name = "Metode: Futures New Gen v3 BTC Bybit - DragonFortune"
        self.strategy_id = 11
        ...
        # ===== Trading parameters =====
        # Base thresholds (from training_results_20260131_205232):
        # - best_precision_threshold=0.52
        # - best_f1_threshold=0.40
        self.prediction_buy_threshold = 0.56
        self.prediction_sell_threshold = 0.50
        self.short_entry_threshold = 0.56
        # Offset applied on top of sell_th to allow shorts when model is biased high
        self.short_entry_offset = 0.04
        self.position_size_pct = 0.89
        ...
        self.use_trade_rate_thresholds = True
        ...
        self.pred_smooth_n = 4   # moving average window for prediction smoothing
        self.pred_history = deque(maxlen=self.pred_smooth_n)

        self.stop_loss_pct = 0.03
        self.take_profit_pct = 0.05
        self.pred_exit_confirm_bars = 3
        ...
        self.min_volatility = 0.002  # 0.2% hourly volatility floor
        self.enable_short = True
        self.rsi_long_threshold = 55
        self.rsi_short_threshold = 50
        self.use_adx_filter = True
        self.min_adx = 30.0
        self.fast_ema = self.EMA(self.symbol, 12, Resolution.Hour)
        self.slow_ema = self.EMA(self.symbol, 36, Resolution.Hour)
    ...
    def OnData(self, data: Slice):
            ...
            # Make short threshold adaptive (tie to sell threshold + offset)
            base_short = float(self.short_entry_threshold)
            offset = float(getattr(self, "short_entry_offset", 0.0) or 0.0)
            self.current_short_entry_threshold = max(base_short, self.current_sell_threshold + offset)
            self.current_short_entry_threshold = max(0.01, min(0.99, self.current_short_entry_threshold))
