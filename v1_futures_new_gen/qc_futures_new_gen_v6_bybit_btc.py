...
class XGBoostTradingAlgorithm(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.objectstore_prefix = "yumna_btc_bybit_v6"
        ...
        self.model_api_url = f"{self.domain}/api/{self.model_version}/futures_new_gen_btc_bybit/model/103"
        self.dataset_summary_api_url = f"{self.domain}/api/{self.model_version}/futures_new_gen_btc_bybit/summary/104"
        ...
        # ===== Strategy meta =====
        self.strategy_name = "Metode: Futures New Gen v6 BTC Bybit - DragonFortune"
        self.strategy_id = 13
        ...
        self.min_volatility = 0.001  # 0.1% hourly volatility floor
    ...
    def UpdateDynamicThresholds(self, pred: float):
        ...
        if not self.use_dynamic_thresholds:
            self.current_buy_threshold = self.prediction_buy_threshold
            self.current_sell_threshold = self.prediction_sell_threshold
        if not self.use_quantile_short_entry:
            self.current_short_entry_threshold = self.short_entry_threshold
        if not self.use_dynamic_thresholds and not self.use_quantile_short_entry:
            return
        ...
        if self.use_quantile_short_entry:
            short_th = float(np.percentile(self.pred_window, self.short_entry_percentile))
    ...
    def TradeLogic(self, pred: float):
        ...
        # Minimum notional guard ($150) -> raise min_order_quantity
        ...
        if pred > self.current_buy_threshold and qty <= 0 and not self.pending_entry and not self.pending_exit and long_ok:
            ...
            if self.min_order_quantity > 0 and abs(order_qty) < self.min_order_quantity:
                order_qty = self.min_order_quantity

            if order_qty == 0.0 and self.min_order_quantity > 0:
                order_qty = self.min_order_quantity
