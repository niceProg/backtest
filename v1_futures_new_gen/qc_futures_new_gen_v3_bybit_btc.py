...
class XGBoostTradingAlgorithm(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.objectstore_prefix = "yumna_btc_bybit_v3"
        ...
        # ===== Trading parameters =====
        self.prediction_buy_threshold = 0.515
        self.prediction_sell_threshold = 0.485
        ...
        self.min_volatility = 0.0015  # 0.15% hourly volatility floor
        self.enable_short = True
        self.rsi_long_threshold = 53
        self.rsi_short_threshold = 47
    ...
    def TradeLogic(self, pred: float):
        ...
        if pred > self.current_buy_threshold and qty <= 0 and not self.pending_entry and not self.pending_exit and long_ok:
            ...
            if self.min_order_quantity > 0 and abs(order_qty) <= (self.min_order_quantity + 1e-12):
                self.Debug(f"BUY skipped: qty {order_qty:.6f} at/below min {self.min_order_quantity}")
                order_qty = 0.0
        ...
        # ENTRY SHORT
            ...
            if self.min_order_quantity > 0 and abs(order_qty) <= (self.min_order_quantity + 1e-12):
                self.Debug(f"SHORT skipped: qty {order_qty:.6f} at/below min {self.min_order_quantity}")
                order_qty = 0.0
