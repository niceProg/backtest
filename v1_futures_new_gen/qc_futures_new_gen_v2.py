...
class XGBoostTradingAlgorithm(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.pred_exit_allow_loss_gap = 0.05
        self.pred_deterioration_delta = 0.08
        self.vol_target = 0.004
        self.min_position_scale = 0.3
        ...
        self.enable_trend_exit = False
        # Trend filter for exits
    ...
    def OnOrderEvent(self, orderEvent: OrderEvent):
        try:
            ...
            if fill_qty > 0:
                ...
                self.Debug(f"[TRADE] ENTRY filled qty={fill_qty:.6f} price={fill_price:.2f}")
            ...
            # SELL filled (exit)
                ...
                self.Debug(f"[TRADE] EXIT filled qty={fill_qty:.6f} price={fill_price:.2f} reason={reason}")
    ...
    def OnData(self, data: Slice):
            ...
            # Time-based stop to avoid long stagnation
            if self.CheckTimeStop(cur_price):
                rsn = self.last_exit_reason or "TIME"
                self.LogOnDataLine(self.Time, cur_price, f"EXIT ({rsn})", f"Exit triggered by {rsn}", pred=pred)
                self.last_trade_bar_time = self.Time
                return

            # Trend flip exit (EMA-based)
            qty = self.Portfolio[self.symbol].Quantity
            if self.enable_trend_exit and qty > 0 and self.fast_ema.IsReady and self.slow_ema.IsReady:
                if self.fast_ema.Current.Value < self.slow_ema.Current.Value:
                    self.pending_exit = True
                    self.last_exit_reason = "TREND_FLIP"
                    self.Liquidate(self.symbol)
                    self.LogOnDataLine(self.Time, cur_price, "EXIT (TREND_FLIP)", "Exit triggered by TREND_FLIP", pred=pred)
                    self.last_trade_bar_time = self.Time
                    return
    ...
    def TradeLogic(self, pred: float):
        ...
        if pred > self.current_buy_threshold and qty <= 0 and not self.pending_entry and not self.pending_exit:
            ...
            if self.entry_price is not None:
                ...
                self.Debug(f"BUY MarketOrder(qty={order_qty:.6f}) pred={pred:.3f} est_entry=NA")
    ...
    def CheckTimeStop(self, price: float) -> bool:
        try:
            if self.pending_exit:
                return True
            qty = self.Portfolio[self.symbol].Quantity
            if qty <= 0 or self.entry_time is None:
                return False
            mh = int(getattr(self, "max_hold_bars", 0) or 0)
            if mh <= 0:
                return False
            if self.Time < (self.entry_time + timedelta(hours=mh)):
                return False
            ...
            self.sendAPI("/signals", {
                ...
                "target_sl": 0,
                "real_tp": 0,
                "real_sl": self.entry_price - price,
                "message": msg
            })
            return True
        except Exception as e:
            self.Error(f"CheckTimeStop error: {e}")
            return False

    def CalculateDynamicPositionPct(self) -> float:
        base = float(self.position_size_pct)
        ...
        if atr_pct <= 0:
            return base
        scale = min(1.0, self.vol_target / atr_pct)
        scale = max(self.min_position_scale, scale)
        return max(0.01, min(0.99, base * scale))

    def _calc_atr(self, period: int) -> float:
        if len(self.price_window) < period + 1:
            return 0.0
        data = list(self.price_window)[-period-1:]
        trs = []
        prev_close = float(data[0]["close"])
        for row in data[1:]:
            high = float(row["high"])
            low = float(row["low"])
