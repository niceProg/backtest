...
class FeatureEngineer:
    ...
    def load_merged_data(self) -> pd.DataFrame:
        ...
        merged_file = datasets_dir / 'merged_6_tables.parquet'

        if not merged_file.exists():
            # Fallback to root directory for backward compatibility
            merged_file = self.output_dir / 'merged_6_tables.parquet'
    ...
    def add_cross_table_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Top account ratio * price return
        if 'ls_top_ratio' in df.columns and 'price_close_return_1' in df.columns:
            df['cross_ls_price'] = df['ls_top_ratio'] * df['price_close_return_1']
