...
class XGBoostTradingAlgorithm(QCAlgorithm):
    ...
    class ZeroFeeModel(FeeModel):
        def GetOrderFee(self, parameters):
            try:
                ...
                currency = "USD"
            return OrderFee(CashAmount(0, currency))
    ...
    def Initialize(self):
        ...
        self.model_api_url = f"{self.domain}/api/{self.model_version}/futures/latest/model"
        self.dataset_summary_api_url = f"{self.domain}/api/{self.model_version}/futures/latest/dataset-summary"
        ...
        # ===== Symbol (Futures first, fallback to spot) =====
        try:
            try:
            ...
            try:
                try:
                    ...
                    self.SetBrokerageModel(BrokerageName.Binance)
                self.crypto = self.AddCryptoFuture("BTCUSDT", Resolution.Hour)
            except Exception:
                self.crypto = self.AddCrypto("BTCUSDT", Resolution.Hour)
                self.Debug("Warning: Futures not available, using spot BTCUSDT")
        ...
        try:
            ...
            pass
        # Use brokerage fees (disable zero-fee override)
        self.enable_zero_fees = False
        if self.enable_zero_fees:
            try:
                self.Securities[self.symbol].FeeModel = self.ZeroFeeModel()
            except Exception:
                pass
        try:
        ...
        # ===== Strategy meta =====
        self.strategy_name = "Metode: Futures v4 - DragonFortune"
        self.strategy_id = 4
        ...
        self.dynamic_threshold_k = 0.85
        self.dynamic_threshold_min_std = 0.004
        self.dynamic_threshold_min_gap = 0.01
        ...
        self.entry_threshold_buffer = 0.005
        self.exit_threshold_buffer = 0.005
        self.volatility_min = 0.001
        self.momentum_min = -0.002
        ...
        # ===== Logging controls (backtest) =====
        self.enable_ondata_logs = False
    ...
    def OnOrderEvent(self, orderEvent: OrderEvent):
        try:
            ...
            # keep payload small; no holdings/unrealized here
    ...
    # ONDATA
    # =========================================================
    def _get_bar_from_slice(self, data: Slice):
        try:
        ...
        try:
            qbars = getattr(data, "QuoteBars", None)
    ...
    def OnData(self, data: Slice):
            ...
            bar = self._get_bar_from_slice(data)
            if bar is None:
                return
    ...
    def _format_time(self, time_dt):
        try:
            if time_dt is None:
                return "NA"
            if hasattr(time_dt, "strftime"):
                return time_dt.strftime("%Y-%m-%d %H:%M:%S")
            if hasattr(time_dt, "ToString"):
                try:
                    return time_dt.ToString("yyyy-MM-dd HH:mm:ss")
                except Exception:
                    return time_dt.ToString()
            return str(time_dt)
        except Exception:
            return "NA"

    def LogOnDataLine(self, time_dt, price, status: str, reason: str, pred: float = None):
        if not getattr(self, "enable_ondata_logs", True):
            return
        try:
            t = self._format_time(time_dt)
            ...
            if pred is not None:
                ...
                self.Debug(msg)
                self.sendAPI("/logs", {"id_method":self.strategy_id, "datetime": ts, "message": msg})
            else:
    ...
    def BuildFeatures(self):
        try:
            ...
            self.last_feature_map = dict(feat_map)
    ...
    def UpdateDynamicThresholds(self, pred: float):
        ...
        mean = float(np.mean(self.pred_window))
        std = float(np.std(self.pred_window))
        if std < self.dynamic_threshold_min_std:
            self.current_buy_threshold = self.prediction_buy_threshold
            self.current_sell_threshold = self.prediction_sell_threshold
            return

        buy_th = mean + self.dynamic_threshold_k * std
        sell_th = mean - self.dynamic_threshold_k * std
    ...
    def TradeLogic(self, pred: float):
        ...
        # ENTRY
        if pred > (self.current_buy_threshold + self.entry_threshold_buffer) and qty <= 0 and not self.pending_entry and not self.pending_exit:
            vol_5 = float(self.last_feature_map.get("price_rolling_vol_5", 0.0))
            ret_5 = float(self.last_feature_map.get("price_close_return_5", 0.0))
            if vol_5 < self.volatility_min or ret_5 <= self.momentum_min:
                return
        ...
        # EXIT by prediction
        elif pred < (self.current_sell_threshold - self.exit_threshold_buffer) and qty > 0 and not self.pending_exit:
    ...
    def sendAPI(self, path: str, params: dict):
        ...
        """
        if not getattr(self, "enable_api_calls", True):
            return False
        try:
