...
class XGBoostTradingAlgorithm(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        try:
            self.lot_size = float(self.Securities[self.symbol].SymbolProperties.LotSize)
        except Exception:
            self.lot_size = 0.0
        ...
        self.use_quantile_short_entry = True
        ...
        self.min_pred_exit_pnl_pct = 0.002
    ...
    def TradeLogic(self, pred: float):
        ...
        if self.lot_size > 0 and price > 0:
            min_qty = (self.min_notional / max(price, 1e-9)) / self.lot_size
            self.min_order_quantity = math.ceil(min_qty) * self.lot_size
        else:
            self.min_order_quantity = 0.0
