#!/usr/bin/env python3
"""
Merge futures trading tables into a unified DataFrame.
...
class TableMerger:
    """Merge futures tables into a unified dataset."""
    ...
    def __init__(self, data_filter: DataFilter, output_dir: str = './output_train'):
        ...
        # Define table priority and merge strategies (6 core tables)
        self.base_table = 'cg_futures_price_history'  # Primary table for time base
        self.tables_info = {
            'cg_futures_price_history': {
                'prefix': 'price',
                'key_cols': ['time', 'exchange', 'symbol', 'interval', 'base_asset'],
    ...
    def standardize_column_names(self, df: pd.DataFrame, table_name: str) -> pd.DataFrame:
        ...
        # Normalize symbol to base asset for futures tables
        if 'base_asset' in df_std.columns:
            df_std['symbol'] = df_std['base_asset']
            df_std = df_std.drop(columns=['base_asset'])
        elif 'symbol' in df_std.columns:
            df_std['symbol'] = df_std['symbol'].map(self._normalize_base_symbol)
    ...
    def merge_table(self, base_df: pd.DataFrame, table_df: pd.DataFrame, table_name: str) -> pd.DataFrame:
        ...
        # Define merge keys per table
        merge_keys = self.tables_info[table_name].get('merge_keys', ['time', 'exchange', 'symbol', 'interval'])

        # Remove duplicates
        table_std = table_std.drop_duplicates(subset=merge_keys, keep='last')
