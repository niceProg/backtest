...
class XGBoostTradingAlgorithm(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        # ===== Strategy meta =====
        self.strategy_name = "Metode: Futures New Gen v1 BTC Bybit - DragonFortune"
        self.strategy_id = 10
        ...
        # ===== Trading parameters =====
        # Looser defaults so backtest can generate trades, then be tightened downstream
        self.prediction_buy_threshold = 0.56
        self.prediction_sell_threshold = 0.45
        self.short_entry_threshold = 0.52
        ...
        self.use_quantile_short_entry = False
        self.short_entry_percentile = 40
        self.pred_window = deque(maxlen=200)
        # Quantile-based thresholds so both long & short can trigger even if preds are biased high
        self.use_quantile_thresholds = False
        self.quantile_min_samples = 30
        self.buy_percentile = 56
        self.sell_percentile = 45
        self.short_percentile = 50
        self.min_threshold_gap = 0.01
        ...
        self.use_trend_filter_short = True
    ...
    def OnData(self, data: Slice):
            ...
            # Update thresholds (quantile-aware to enable both long and short)
            self.UpdateThresholds(pred)
            if not getattr(self, "_config_banner_logged", False):
                ...
                self.Debug(
                    ...
                    f"quantile_th={self.use_quantile_thresholds} "
                    f"quantile_short={self.use_quantile_short_entry} "
                    f"adx_filter={self.use_adx_filter} trend_filter={self.use_trend_filter} "
            ...
            if (not pre_entry) and self.pending_entry:
                ...
                if self.entry_direction == -1:
                    reason = f"pred={pred:.3f} < sell_th={self.current_sell_threshold:.2f} -> submit SHORT"
                else:
    ...
    # =========================================================
    # SHORT ENTRY THRESHOLD (quantile-based)
    # =========================================================
    def UpdateThresholds(self, pred: float):
        """Update buy/sell/short thresholds with optional quantile logic."""
        use_q = bool(getattr(self, "use_quantile_thresholds", False))
        if not use_q:
            # Static defaults + optional short-only quantile
            self.current_buy_threshold = self.prediction_buy_threshold
            self.current_sell_threshold = self.prediction_sell_threshold
            self.current_short_entry_threshold = self.short_entry_threshold
            self.UpdateShortEntryThreshold(pred)
            return

        try:
            self.pred_window.append(float(pred))
        except Exception:
            # Fall back to static thresholds on any issue
            self.current_buy_threshold = self.prediction_buy_threshold
            self.current_sell_threshold = self.prediction_sell_threshold
            self.current_short_entry_threshold = self.short_entry_threshold
            return

        min_samples = int(getattr(self, "quantile_min_samples", 30) or 30)
        if len(self.pred_window) < min_samples:
        ...
        buy_pct = float(getattr(self, "buy_percentile", 70) or 70)
        sell_pct = float(getattr(self, "sell_percentile", 45) or 45)
        short_pct = float(getattr(self, "short_percentile", 30) or 30)
        min_gap = float(getattr(self, "min_threshold_gap", 0.01) or 0.01)

        buy_th = float(np.percentile(self.pred_window, buy_pct))
        sell_th = float(np.percentile(self.pred_window, sell_pct))
        short_th = float(np.percentile(self.pred_window, short_pct))

        # Enforce ordering with a minimum gap so buy doesn't always dominate
        if sell_th >= buy_th - min_gap:
            sell_th = buy_th - min_gap
        if short_th >= sell_th - min_gap:
            short_th = sell_th - min_gap

        # Clamp into sane bounds
        buy_th = max(0.01, min(0.99, buy_th))
        sell_th = max(0.01, min(0.99, sell_th))
        short_th = max(0.01, min(0.99, short_th))

        self.current_buy_threshold = buy_th
        self.current_sell_threshold = sell_th
        self.current_short_entry_threshold = short_th

    def UpdateShortEntryThreshold(self, pred: float):
        if not getattr(self, "use_quantile_short_entry", False):
            self.current_short_entry_threshold = self.short_entry_threshold
            return

        try:
            self.pred_window.append(float(pred))
            if len(self.pred_window) < 30:
                self.current_short_entry_threshold = self.short_entry_threshold
                return
            pct = float(getattr(self, "short_entry_percentile", 40))
            short_th = float(np.percentile(self.pred_window, pct))
    ...
    def TradeLogic(self, pred: float):
        ...
        # No indicator filter (predict-only)
        ind_ready = False
        long_ok = True
        short_ok = True
        ...
        if self.use_trend_filter_short:
            if not (self.fast_ema.IsReady and self.slow_ema.IsReady):
                return
